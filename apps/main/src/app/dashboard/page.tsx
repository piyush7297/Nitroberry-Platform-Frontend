...
export default function DashboardStats() {
  ...
  // type: 1=day, 2=week, 3=month — matches API kanBanType/type param
  const kanbanViewType = calendarViewType === "day" ? 1 : calendarViewType === "week" ? 2 : 3;

  // Kanban date params based on current view and selected date
  const kanbanDateParams = (() => {
    const isKanban = viewMode === "kanban";
    if (!isKanban) return "";
    if (calendarViewType === "day") {
      const d = format(selectedDate, "yyyy-MM-dd");
      return `&isKanban=true&type=${kanbanViewType}&startDate=${d}&endDate=${d}`;
    }
    if (calendarViewType === "week") {
      const start = format(startOfWeek(selectedDate), "yyyy-MM-dd");
      const end = format(addDays(startOfWeek(selectedDate), 6), "yyyy-MM-dd");
      return `&isKanban=true&type=${kanbanViewType}&startDate=${start}&endDate=${end}`;
    }
    // month
    const month = selectedDate.getMonth() + 1;
    const year = selectedDate.getFullYear();
    return `&isKanban=true&type=${kanbanViewType}&month=${month}&year=${year}`;
  })();

  // Task-specific kanban param key name is kanBanType (from Postman)
  const taskKanbanDateParams = (() => {
    const isKanban = viewMode === "kanban";
    if (!isKanban) return "";
    if (calendarViewType === "day") {
      const d = format(selectedDate, "yyyy-MM-dd");
      return `&isKanban=true&kanBanType=${kanbanViewType}&startDateKanban=${d}&endDateKanban=${d}`;
    }
    if (calendarViewType === "week") {
      const start = format(startOfWeek(selectedDate), "yyyy-MM-dd");
      const end = format(addDays(startOfWeek(selectedDate), 6), "yyyy-MM-dd");
      return `&isKanban=true&kanBanType=${kanbanViewType}&startDateKanban=${start}&endDateKanban=${end}`;
    }
    const month = selectedDate.getMonth() + 1;
    const year = selectedDate.getFullYear();
    return `&isKanban=true&kanBanType=${kanbanViewType}&month=${month}&year=${year}`;
  })();
  ...
  // View-more day state — when set, triggers a day-specific API fetch
  const [viewMoreDay, setViewMoreDay] = useState<{ date: Date | null }>({ date: null });

  // Helper: flatten kanban keyed response { "YYYY-MM-DD": { count, list } } → { items, countByDate }
  const parseKanbanResponse = (responseData: any): { items: any[]; countByDate: Map<string, number> } => {
    const kanbanData = responseData?.steps ?? responseData?.drafts ?? responseData?.tasks ?? responseData?.task ?? responseData?.data ?? responseData;
    const countByDate = new Map<string, number>();
    if (kanbanData && typeof kanbanData === "object" && !Array.isArray(kanbanData)) {
      const items: any[] = [];
      Object.entries(kanbanData).forEach(([dateKey, bucket]: [string, any]) => {
        if (bucket && typeof bucket === "object" && "list" in bucket) {
          countByDate.set(dateKey, bucket?.count ?? 0);
          (bucket?.list || []).forEach((item: any) => items.push(item));
        }
      });
      if (items.length > 0 || countByDate.size > 0) return { items, countByDate };
    }
    // Fallback: plain array response (table view)
    return { items: Array.isArray(kanbanData) ? kanbanData : [], countByDate };
  };
  ...
  const {
    ...
    ["USER_STEP_LIST", start, limit, status.join(","), dashboardMode, viewMode, calendarViewType, selectedDate.toISOString()],
    `${API_ENDPOINTS.FMS_STEP}/assigned-list?start=${start}&limit=${limit}&status=${status.join(",")}${isAdminParam}${kanbanDateParams}`,
    {
  ...
  const {
    ...
    ["TASKS", taskStart, taskLimit, activeTab, dashboardMode, viewMode, calendarViewType, selectedDate.toISOString()],
    `${API_ENDPOINTS.TASKS}?start=${taskStart}&limit=${taskLimit}&status=1,3&type=${
      activeTab === TaskTypeEnum.DELEGATION
        ? TaskTypeEnum.DELEGATION
        : `${TaskTypeEnum.HELP},${TaskTypeEnum.RECURRING}`
    }&createdByTask=false&sortBy=createdAt&sortOrder=desc${isAdminParam}${taskKanbanDateParams}`,
    {
      refetchOnWindowFocus: false,
      retry: 1,
      staleTime: 0,
  ...
  const {
    ...
    [`DRAFT_INDENT_TASKS-${draftIndentStart}-${draftIndentLimit}-${draftIndentStatus.join(",")}-${dashboardMode}-${viewMode}-${calendarViewType}-${selectedDate.toISOString()}`],
    `${API_ENDPOINTS.FMS_INDENT_DRAFT_LIST}?sortBy=createdAt&sortOrder=asc&start=${draftIndentStart}&limit=${draftIndentLimit}&status=${draftIndentStatus.join(",")}${isAdminParam}${kanbanDateParams}`,
    {
      refetchOnWindowFocus: false,
      retry: 1,
      staleTime: 0,
  ...
  // Parse kanban keyed responses
  const { items: workflowItems, countByDate: workflowCountByDate } = parseKanbanResponse(userStep?.data);
  const { items: draftItems, countByDate: draftCountByDate } = parseKanbanResponse(draftIndentTasksData?.data);
  const { items: taskItems, countByDate: taskCountByDate } = parseKanbanResponse(tasksData?.data);

  // Day-specific view-more date params
  const viewMoreDayStr = viewMoreDay.date ? format(viewMoreDay.date, "yyyy-MM-dd") : "";
  const viewMoreDayParams = viewMoreDay.date
    ? `&isKanban=true&type=1&startDate=${viewMoreDayStr}&endDate=${viewMoreDayStr}`
    : "";
  const viewMoreDayTaskParams = viewMoreDay.date
    ? `&isKanban=true&kanBanType=1&startDateKanban=${viewMoreDayStr}&endDateKanban=${viewMoreDayStr}`
    : "";

  const { data: viewMoreStepsData, isLoading: isViewMoreStepsLoading } = useApiQuery(
    ["VIEW_MORE_STEPS", viewMoreDayStr, dashboardMode],
    `${API_ENDPOINTS.FMS_STEP}/assigned-list?start=1&limit=100&status=${status.join(",")}${isAdminParam}${viewMoreDayParams}`,
    { enabled: !!viewMoreDay.date && activeTab === TaskTypeEnum.FLOW, refetchOnWindowFocus: false, retry: 1 } as const,
  );

  const { data: viewMoreDraftData, isLoading: isViewMoreDraftLoading } = useApiQuery(
    ["VIEW_MORE_DRAFT", viewMoreDayStr, dashboardMode],
    `${API_ENDPOINTS.FMS_INDENT_DRAFT_LIST}?start=1&limit=100&status=${draftIndentStatus.join(",")}${isAdminParam}${viewMoreDayParams}`,
    { enabled: !!viewMoreDay.date && activeTab === TaskTypeEnum.DRAFT_INDENT, refetchOnWindowFocus: false, retry: 1 } as const,
  );

  const { data: viewMoreTasksData, isLoading: isViewMoreTasksLoading } = useApiQuery(
    ["VIEW_MORE_TASKS", viewMoreDayStr, activeTab, dashboardMode],
    `${API_ENDPOINTS.TASKS}?start=1&limit=100&status=1,3&type=${activeTab === TaskTypeEnum.DELEGATION ? TaskTypeEnum.DELEGATION : `${TaskTypeEnum.HELP},${TaskTypeEnum.RECURRING}`}&createdByTask=false${isAdminParam}${viewMoreDayTaskParams}`,
    { enabled: !!viewMoreDay.date && (activeTab === TaskTypeEnum.DELEGATION || activeTab === TaskTypeEnum.MY_TASKS), refetchOnWindowFocus: false, retry: 1 } as const,
  );

  // Flatten view-more day responses
  const viewMoreStepItems = parseKanbanResponse(viewMoreStepsData?.data).items;
  const viewMoreDraftItems = parseKanbanResponse(viewMoreDraftData?.data).items;
  const viewMoreTaskItemsAll = parseKanbanResponse(viewMoreTasksData?.data).items;
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8">
      ...
      <div className="space-y-6">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            {activeTab === TaskTypeEnum.FLOW && (
              <CalendarKanban
                ...
                items={workflowItems.length > 0 ? workflowItems : dummyKanbanData[TaskTypeEnum.FLOW]}
                countByDate={workflowCountByDate.size > 0 ? workflowCountByDate : undefined}
                getDate={(item) => item.date || item.startDate || item.createdAt || '2024-04-18'}
                renderCard={(item) => (
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-[10px] text-gray-500">
                        <span>Priority</span>
                        <span className={cn("rounded-full px-1.5 py-0.5 font-bold uppercase", item.priority === 'High' ? "bg-rose-100 text-rose-700" : "bg-blue-100 text-blue-700")}>
                ...
                onViewMore={(date) => setViewMoreDay({ date })}
              />
            )}
            ...
            {activeTab === TaskTypeEnum.DRAFT_INDENT && (
              <CalendarKanban
                viewType={calendarViewType}
                selectedDate={selectedDate}
                items={draftItems.length > 0 ? draftItems : dummyKanbanData[TaskTypeEnum.DRAFT_INDENT]}
                countByDate={draftCountByDate.size > 0 ? draftCountByDate : undefined}
                getDate={(item) => item.date || item.startDate || item.createdAt || '2024-04-18'}
                renderCard={(item) => (
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between border-t border-slate-100 pt-2">
                      ...
                      <span className={cn("text-[9px] font-bold px-1.5 rounded-md", item.status === 'approved' ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700")}>
            ...
            {(activeTab === TaskTypeEnum.DELEGATION || activeTab === TaskTypeEnum.MY_TASKS) && (
              <CalendarKanban
                viewType={calendarViewType}
                selectedDate={selectedDate}
                items={taskItems.length > 0 ? taskItems : dummyKanbanData[activeTab as keyof typeof dummyKanbanData]}
                countByDate={taskCountByDate.size > 0 ? taskCountByDate : undefined}
                getDate={(item) => item.dueDate || item.date || item.startDate || item.createdAt || '2024-04-18'}
                renderCard={(item) => (
                  <div className="space-y-3">
                    <h3 className="font-semibold text-base text-gray-900 mb-2 capitalize group-hover:text-primary transition-colors line-clamp-2">
                      {item.title}
        ...
        {/* View More Side Drawer — fetches day-specific data from API */}
        {(() => {
          const isOpen = !!viewMoreDay.date;
          const dateLabel = viewMoreDay.date?.toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" }) ?? "";
          const isLoading = isViewMoreStepsLoading || isViewMoreDraftLoading || isViewMoreTasksLoading;
          const items =
            activeTab === TaskTypeEnum.FLOW ? viewMoreStepItems
            : activeTab === TaskTypeEnum.DRAFT_INDENT ? viewMoreDraftItems
            : viewMoreTaskItemsAll;
            ...
            activeTab === TaskTypeEnum.FLOW
              ...
              : (item: any) => (
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-5 w-5 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-600 border border-slate-200">{item.assignee?.[0] || 'U'}</div>
          ...
          const onItemClick =
            activeTab === TaskTypeEnum.FLOW
              ? (item: any) => { if (item.stepId) router.push(`${RoutesEnum.WORKFLOW_STEPS}/${item.stepId}`); }
              : activeTab === TaskTypeEnum.DRAFT_INDENT
              ? (item: any) => { if (item.id) router.push(`/dashboard/fms-indents/draft-list/${item.id}`); }
              : (item: any) => { if (item.id) router.push(`/dashboard/task-detail/${item.id}`); };

          return (
            <Sheet open={isOpen} onOpenChange={(open) => { if (!open) setViewMoreDay({ date: null }); }}>
              <SheetContent side="right" className="w-[96vw] gap-0 p-0 sm:max-w-xl">
                <div className="flex h-full flex-col">
                  <SheetHeader className="border-b px-6 py-4">
                    <SheetTitle className="text-base font-bold">{TAB_TYPE_TO_QUERY[activeTab as TaskTypeEnum]?.replace('-', ' ')} — {dateLabel}</SheetTitle>
                    <SheetDescription className="text-xs text-slate-500">
                      {isLoading ? "Loading..." : `${items.length} ${items.length === 1 ? "item" : "items"} on this day`}
                    </SheetDescription>
                  </SheetHeader>
                  <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                    {isLoading ? (
                      Array.from({ length: 4 }).map((_, i) => (
                        <div key={i} className="rounded-xl border border-slate-200 bg-white p-4 animate-pulse space-y-2">
                          <div className="h-4 bg-gray-200 rounded w-3/4" />
                          <div className="h-3 bg-gray-100 rounded w-1/2" />
                        </div>
                      ))
                    ) : items.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-16 text-slate-400 text-sm">No items for this day.</div>
                    ) : (
                      items.map((item, idx) => (
                        <div
                          key={item.id || idx}
                          onClick={() => { onItemClick(item); setViewMoreDay({ date: null }); }}

...
function SortableStepItem({ step, onEdit, index, onDelete, onPreview }: any) {
  ...
  const assignedCount = step?.assignment?.userIds?.length || 0;
  const assignedNames = (step?.assignment?._fullnames || step?.assignment?.fullnames || []).filter(Boolean);
...
export default function WorkflowDesign({
  ...
  const normalizeStepForCompare = (step: any) => ({
    ...
    assignment: Array.isArray(step?.assignment)
      ? {
          userIds: step.assignment.flatMap((a: any) => a.userId || a.userIds || []),
          _fullnames: step.assignment.flatMap((a: any) => a.fullname || []),
        }
      : (step?.assignment ?? { userIds: [], _fullnames: [] }),
    escalation: step?.escalation
      ? {
          userIds: step.escalation.userIds ?? [],
          _fullnames: step.escalation.fullnames ?? step.escalation._fullnames ?? [],
          frequencyInMinutes: step.escalation.frequencyInMinutes ?? 0,
          notifyInDelay: step.escalation.notifyInDelay ?? 0,
        }
      : { userIds: [], _fullnames: [], frequencyInMinutes: 0, notifyInDelay: 0 },
  ...
  const hasPendingStepDraft = () => {
    const hasDraftContent =
      ...
      (currentStep?.assignment?.userIds?.length || 0) > 0;
  ...
  const addWorkflowRule = () => {
    ...
    setFormData((prev: any) => {
      ...
      const assignment = {
        userIds: currentStep.assignment?.userIds ?? [],
      };
  ...
  const getStepAssigneeNames = (step: any) => {
    ...
    if (!a) return [];
    if (Array.isArray(a)) return a.flatMap((x: any) => x.fullname || []).filter(Boolean);
    return (a._fullnames || a.fullnames || []).filter(Boolean);
  };
  const getStepEscalationNames = (step: any) => {
    const names = step?.escalation?._fullnames || step?.escalation?.fullnames;
  ...
  const totalStepAssignees = (formData?.workflowTemplate?.steps || []).reduce(
    (sum: number, item: any) => {
      ...
      if (!a) return sum;
      if (Array.isArray(a)) return sum + a.reduce((inner: number, x: any) => inner + (x?.userId?.length || 0), 0);
      return sum + (a.userIds?.length || 0);
    },
    0,
  );
        ...
        <div className="md:col-span-7 space-y-4">
          <div className="space-y-2">
            <UserSearch
              ...
              selectedUserIds={currentStep.assignment?.userIds || []}
              onSelect={(user: any) => {
                const fullname = `${user.firstName} ${user.lastName}`;
                const prev = currentStep.assignment || { userIds: [], _fullnames: [] };
                setCurrentStep({
                  ...currentStep,
                  assignment: {
                    userIds: [...(prev.userIds || []), user.id],
                    _fullnames: [...(prev._fullnames || []), fullname],
                  },
                });
              }}
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {(currentStep.assignment?.userIds || []).length > 0 ? (
                (currentStep.assignment.userIds || []).map((uid: string, userIndex: number) => {
                  const fullname = currentStep.assignment._fullnames?.[userIndex] || null;
                  return (
                    <span
                      key={uid}
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          const prev = currentStep.assignment;
                          setCurrentStep({
                            ...currentStep,
                            assignment: {
                              userIds: prev.userIds.filter((_: string, i: number) => i !== userIndex),
                              _fullnames: (prev._fullnames || []).filter((_: string, i: number) => i !== userIndex),
                            },
                          });
                        }}
                  ...
                  {(() => {
                    const userIdsArray = Array.isArray(step.escalation?.userIds)
                      ...
                      : step.escalation?.userIds
                        ? [step.escalation.userIds]
                        : [];
                    const rawNames = step.escalation?._fullnames ?? step.escalation?.fullnames;
                    const fullnamesArray = Array.isArray(rawNames) ? rawNames : rawNames ? [rawNames] : [];
                          ...
                          onSelect={(user: any) => {
                            ...
                            setCurrentStep({
                              ...
                              escalation: {
                                ...
                                _fullnames: [...fullnamesArray, fullname],
                              },
                            });
                          }}
                        />
                                      ...
                                      onClick={() =>
                                        setCurrentStep({
                                          ...
                                          escalation: {
                                            ...
                                            _fullnames: fullnamesArray.filter(
                                              (_: string, index: number) =>
                                                index !== userIndex,
                                            ),
                                          },
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-medium text-slate-900">{previewStepModal.escalation?.userIds?.length > 0 ? `${previewStepModal.escalation.userIds.length} user(s)` : 'Not set'}</p>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm">
                            <div className="grid gap-3 sm:grid-cols-2">
                              ...
                              <div>
                                <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Escalation Settings</p>
                                <p className="mt-1 font-medium text-slate-900">{activeWorkflowPreviewStep.escalation?.userIds?.length > 0 ? `${activeWorkflowPreviewStep.escalation.userIds.length} user(s)` : 'Not set'}</p>
                                <p className="text-xs text-slate-500">

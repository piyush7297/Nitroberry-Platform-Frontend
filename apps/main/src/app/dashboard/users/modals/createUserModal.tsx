...
const CreateUserForm: React.FC<Props> = ({
  ...
  const selectedLocationId = formik.values.locationId || "";
  const { data: shiftsData } = useApiQuery(
    ["COMPANY_SHIFTS_create_user", selectedLocationId],
    `${API_ENDPOINTS.COMPANY_SHIFT}?start=1&limit=1000${selectedLocationId ? `&locationId=${selectedLocationId}` : ""}`,
    { enabled: !!selectedLocationId, refetchOnWindowFocus: false } as const,
  );
          ...
          <div className="flex flex-col space-y-2">
            ...
            >
              <SelectTrigger id="shiftId" className="w-full">
                <SelectValue placeholder={selectedLocationId ? "Select Shift" : "Select a location first"} />
              </SelectTrigger>
              <SelectContent>

...
export default function UsersTable() {
  ...
  useEffect(() => {
    ...
    if (pathname === "/dashboard/users") {
      router.replace(`/dashboard/setup?${query}`, { scroll: false });
      return;
    }

    if (pathname === "/dashboard/setup" && query !== searchParams.toString()) {
      router.replace(`/dashboard/setup?${query}`, { scroll: false });
    }
  ...
  const {
    ...
    `${API_ENDPOINTS.STATISTICS_SETUP}?type=${activeStatisticsType}`,
    {
      ...
      retry: 0,
      enabled: Boolean(activeStatisticsType) && pathname === "/dashboard/setup",
    } as const,
  );
  ...
  const { data: holidayListData } = useApiQuery(
    ["HOLIDAY_SCOPE_LIST"],
    `${API_ENDPOINTS.COMPANY_HOLIDAY}?start=1&limit=1000`,
    {
      enabled: shouldFetchUserFormOptions,
      refetchOnWindowFocus: false,
      retry: 1,
      ...
      <div className="min-w-0 p-4 sm:p-6 space-y-4 sm:space-y-6 overflow-x-hidden">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col">
            ...
            <p className="text-gray-600">
              Configure and manage system users, roles, groups, departments, and access permissions.
            </p>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className={cn(pageTabsTrackClassName, "gap-2")}>
              {isTabsOverflowing && (
                <Button
                  type="button"
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 shrink-0 rounded-md border border-primary/30 text-primary hover:bg-primary/10"
                ...
                >
                  {tabs.map((item, index) => (
                    ...
                    >
                      ...
                      {item.value === "plan" && (
                        <span
                          className={pageTabBadgeClassName(activeTab === item.value)}
                        >
                          2
                        </span>
                      )}

...
export interface ModuleActionPermissions {
  ...
  delete: boolean;
  viewall: boolean;
  [key: string]: boolean; // Support for "export", "import", etc.
}
...
export const PermissionsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const {
    ...
    `${API_ENDPOINTS.COMMON_ROLES_PERMISSIONS}?start=1&limit=100&type=1`,
    {
      ...
      staleTime: Infinity, // ensures data is fetched only once and stored in cache
      gcTime: Infinity, // prevents garbage collection as long as the session lives
    ...
    roles.forEach((role: any) => {
      ...
      if (role.permissions && role.permissions.length > 0) {
        role.permissions.forEach((p: any) => {
          ...
          if (action) {
            permissionsMap[moduleId][action] = p.isAllowed === true;
          }
        });
      }
    });

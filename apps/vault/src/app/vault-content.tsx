...
export default function VaultPage() {
  ...
  const { data: vaultListData, refetch: refetchVault, isLoading: isVaultFetching } = useApiQuery(
    ["VAULT_LIST"],
    API_ENDPOINTS.VAULT,
    {
      enabled: isUnlocked && !!privateKey,
      refetchOnWindowFocus: false,
      retry: false,
    } as any,
    { headers: { "x-vault-key": privateKey } },
  );

  useEffect(() => {
    if (vaultListData) {
      const rawEntries = vaultListData.data?.vaultList || vaultListData.data?.data?.vaultList || vaultListData.data || [];
      setAllEntries(Array.isArray(rawEntries) ? rawEntries.map(mapVaultEntry) : []);
    }
  }, [vaultListData]);
  ...
  const vaultTotal = filteredEntries.length;
  const entries = useMemo(() => {
    const pageStart = (currentPage - 1) * PAGE_LIMIT;
    return filteredEntries.slice(pageStart, pageStart + PAGE_LIMIT);
  ...
  const handleUnlockVault = async () => {
    ...
    setIsBusy(true);
    try {
      await apiCall("get", API_ENDPOINTS.VAULT_VERIFY_KEY, undefined, {
        headers: { "x-vault-key": normalizedUnlockKey },
      });
      persistPrivateKey(normalizedUnlockKey);
      setIsUnlocked(true);
      setGeneratedSecrets(null);
      toast({ title: "Vault unlocked", description: "Your vault is now accessible.", variant: "default" });
    } catch (err: any) {
      const msg = err?.response?.data?.message || "Invalid private key. Please try again.";
  ...
  return (
    <div className="min-w-0 p-4 sm:p-6 space-y-4 sm:space-y-6 overflow-x-hidden">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col">
          <div className="flex items-center gap-2 mb-2">
            <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[10px] font-medium text-primary">
        ...
        <div className="flex flex-col items-center justify-center py-12 px-4 relative">
          ...
          <div className="w-full max-w-md space-y-8 transition-all duration-500 relative">
            <div className="text-center space-y-2">
              <div className="inline-flex h-20 w-20 items-center justify-center rounded-3xl bg-primary/10 text-primary shadow-inner mb-4">
            ...
            <Card className="border-slate-200/60 shadow-xl shadow-slate-200/50 bg-white/80 backdrop-blur-sm overflow-hidden">
              <CardContent className="p-8 space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      className="h-12 bg-slate-50/50 border-slate-200 focus:bg-white transition-all pr-12 [&::-ms-reveal]:hidden [&::-webkit-credentials-auto-fill-button]:hidden"
                    />
                    <button
                      type="button"
                      onClick={() => setShowUnlockPassword((v) => !v)}
                      className="absolute inset-y-0 right-3 flex items-center text-muted-foreground hover:text-foreground"
                      aria-label={showUnlockPassword ? "Hide private key" : "Show private key"}
                    >
                ...
                <Button
                  className="w-full h-12 text-sm font-semibold shadow-lg shadow-primary/20 group"
          ...
          <div className="space-y-4">
            <Card className="overflow-hidden border-slate-200/80 bg-white shadow-sm">
              <CardContent className="p-0">
                ...
                {!isVaultFetching && paginatedEntries.length > 0 && vaultTotal > PAGE_LIMIT && (
        ...
        <SheetContent className="sm:max-w-xl border-l border-slate-200 bg-white p-0">
          <SheetHeader className="p-10 pb-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20">Vault Secure</Badge>
            </div>
          ...
          <div className="p-10 bg-white border-t border-slate-100">
            <div className="flex gap-4">
              ...
              <Button className="flex-1 bg-primary hover:bg-primary/90 h-11" onClick={handleSaveEntry}>
      ...
      <Sheet open={openShareDrawer} onOpenChange={setOpenShareDrawer}>
        ...
        >
          <SheetHeader className="border-b border-slate-200/80 bg-gradient-to-b from-primary/10 via-white to-white px-6 py-5">
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-2">
                <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-white px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-primary shadow-sm">
                      ...
                      {entries.map((entry) => {
                        ...
                        return (
                          <div
                            ...
                            onKeyDown={(e) => e.key === "Enter" && toggleShareEntry(entry.id)}
                            className={`w-full cursor-pointer rounded-2xl border p-4 text-left transition ${isSelected ? "border-primary/20 bg-primary/5 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`}
                          >
                      ...
                      liveTeammates.map((member: any) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left transition ${isSelected ? "border-primary/20 bg-primary/5 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`}
                          >
                            <Avatar className="h-10 w-10 border border-slate-200">
                              <AvatarFallback className="bg-gradient-to-br from-primary/20 to-primary/5 text-[11px] font-bold text-primary">
                            ...
                            <Check className={`h-4 w-4 shrink-0 ${isSelected ? "text-primary opacity-100" : "text-slate-300 opacity-100"}`} />
                          </button>
                        );
                      })
                    ) : (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="h-11 border-slate-200 bg-slate-50 pl-9 text-sm focus:border-primary/40 focus:bg-white"
                      />
                    </div>
                  </div>
          ...
          <SheetFooter className="border-t border-slate-200 bg-white px-6 py-4">
            ...
            <Button className="flex-1 bg-primary hover:bg-primary/90" onClick={handleShare} disabled={shareRecipients.length === 0 || !sharePrivateKey}>

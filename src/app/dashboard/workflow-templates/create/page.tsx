...
const normalizeTemplateForCreate = (fms: any) => {
  ...
  return {
    ...
    workflowTemplate: {
      ...
      priority: fms?.workflowTemplate?.priority ?? defaults.workflowTemplate.priority,
      skipHolidays:
        fms?.workflowTemplate?.skipHolidays ??
        defaults.workflowTemplate.skipHolidays,
      businessHours:
        fms?.workflowTemplate?.businessHours ||
        defaults.workflowTemplate.businessHours,
    },
  };
};
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8">
      ...
      {step === 2 && (
        <FormBuilder
          ...
          updateForm={updateForm}
          nextStep={() => {
            if (formData.templeteCode.length < 2) {
              toast({
                title: "Error!",
                description:

...
import type { PagType } from "./interfaces";
...
export const ConfigValue = {
  ...
  AUTH_TOKEN_KEY: process.env.NEXT_PUBLIC_AUTH_TOKEN_KEY ?? "auth_token",
  AUTH_CRED: "AUTH_CRED",
};
...
export function getBadgeVariant(status?: any): any {
  ...
  switch (s) {
    case 1: case "1": return "secondary";
    case 2: case "2": return "warning";
    case 3: case "3": return "active";
    case 4: case "4": return "info";
    case 5: case "5": return "warning";
    case 6: case "6": return "destructive";
    case 7: case "7": return "destructive";
    case 8: case "8": return "secondary";
...
type StatusConfig = { name: string; variant: string };
type TableStatusMap = { [table: string]: { [statusKey: string]: StatusConfig } };
...
export function getStatusInfo(table: STATUSTABLE, status: StatusKey) {
  ...
  if (!tableConfig) return { name: key, variant: "default" };
  const info = tableConfig[key];
  if (!info) return { name: key, variant: "default" };
  return info;
}

export function getReportPagination(selectedType: number, rawData: any): PagType {
